...
public final class MesheliumLifecycleTortureTest implements FabricClientGameTest {
    ...
    private static void assertNoErrors() {
        ...
        // SEAM steps 3-4 structural invariants, checked after EVERY leg
        // because this suite is the one that exercises the transitions
        // (reloads, rd changes, renderer swaps, world hops) where the
        // deleted mechanisms used to fire. ledgerDemoted's producers (the
        // deadline expiry, the dead-successor demote) were deleted with
        // the deadlines, so ANY movement is a resurrected mechanism; an
        // illegal ledger transition is an ownership bug by the table's
        // contract. This is the successor to the design's
        // "farHandoverArmRefusedRingDown must go structurally zero" -
        // that counter died with the arming chain it was counting, and
        // zero demotes through a stand-down IS the fact it measured.
        long demoted = TerrainResidency.ledgerDemoted();
        if (demoted != 0) {
            throw new AssertionError("ledgerDemoted = " + demoted
                    + " - the AWAITING -> NEAR_HELD edge is DEAD since seam step 4 "
                    + "(no deadlines, no dead-successor demote); something rebuilt a "
                    + "producer for it");
        }
        long illegal = TerrainResidency.ledgerIllegalTransitions();
        if (illegal != 0) {
            throw new AssertionError("ledgerIllegalTransitions = " + illegal
                    + " - a transition outside the ownership ledger's legal table; "
