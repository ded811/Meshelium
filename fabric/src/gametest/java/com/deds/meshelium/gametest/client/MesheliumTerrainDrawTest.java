...
public final class MesheliumTerrainDrawTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        try (TestSingleplayerContext singleplayer = context.worldBuilder().create()) {
            ...
            // THE COMPILE TAP IS ATTACHED. Everything above proves we are
            // DRAWING; this proves we are being FED. The two are separable
            // and the gap between them is this renderer's worst failure:
            // SectionBuildTap.onCompileReturn hangs off one pinned
            // SectionCompiler.compile descriptor, and if that stops
            // matching the mixin still applies, defaultRequire is still
            // satisfied, nothing is logged, and the arena simply stays
            // empty while vanilla draws a normal-looking world. A suite
            // that only checks frames and errors goes green through it.
            //
            // Not hypothetical: NeoForge 26.2 keeps the 4-arg compile as a
            // deprecated shim and repoints its only caller at a 5-arg
            // overload, which disconnects this tap by construction. This
            // assertion is what turns that from a silent no-draw into a
            // failed test naming the mixin.
            long tapHits = SectionBuildTap.compileTapHits();
            if (tapHits == 0L) {
                throw new AssertionError("the section-compile tap never fired while "
                        + TerrainDrawer.framesDrawn() + " frames drew "
                        + TerrainDrawer.lastDrawnSections() + " sections: vanilla is building "
                        + "terrain and SectionCompiler.compile is not being seen, so nothing "
                        + "is decoded and nothing reaches the arena. Check "
                        + "SectionCompilerMixin's pinned descriptor against this build "
                        + "(orphaned mesh constructions: " + SectionBuildTap.meshTapOrphans() + ")");
            }
    ...
    private static void assertScalingPinnedFromOption(ClientGameTestContext context) {
        context.runOnClient(client -> {
            ...
            // pre13: maxRegions has a SECOND input now - the far-field
            // ring's region columns (MesheliumScaling.farRegionDemand) -
            // so the standard-pin proof asserts the standard budget only
            // when the far ring is not asking for more. Everything else
            // here is unchanged, and `extended` in particular must STILL
            // be false: a far bump raises the id budget and nothing else,
            // which is the whole point of keeping the two apart.
            int farDemand = com.deds.meshelium.MesheliumScaling.farRegionDemand();
            int expectRegions = Math.max(
                    com.deds.meshelium.MesheliumScaling.STANDARD_MAX_REGIONS, farDemand);
            if (pinned.extended()
                    || pinned.maxRegions() != expectRegions
                ...
                throw new AssertionError("world at a <=32 option under ceiling " + ceiling
                        + " must pin the STANDARD sizes (region budget "
                        + expectRegions + ", far demand " + farDemand + "), got " + pinned
    ...
    private static void assertLiveRangeApplication(ClientGameTestContext context) {
        ...
        context.runOnClient(client -> {
            ...
            try {
                ...
                var codec = ((com.deds.meshelium.mixin.OptionInstanceAccessor)
            ...
            try {
                ...
                try {
                    context.waitFor(client -> TerrainDrawer.bfsOnlyFrames() > bfsBefore2,
                            DRAW_TIMEOUT_TICKS);
                    // Far shells set the SAME RegionStore retained bit (the
                    // BFS path has to dispatch far-only regions), so they
                    // ride this probe too. With the far field armed a
                    // non-zero count is expected, not a retention leak.
                    if (!com.deds.meshelium.farfield.FarFieldConfig.enabled()
                            && TerrainDrawer.lastRetainedMaskSections() != 0) {
