...
public final class GreedyMesher {
    ...
    // ------------------------------------------------------------------
    // What the merge is WORTH, which was going unmeasured entirely.
    //
    // The 5.9 / 18.0 percent figures in the class javadoc are from one
    // offline study of the near field and there has never been a live
    // counter of either field, so every quad-cost claim about a far-field
    // change - the pre12 Smooth Lighting row's included - has been an
    // extrapolation of a near-field number with nothing to check it
    // against. These four adders are that check. Two per merge CALL, i.e.
    // once per bucket per section and never inside the sweep, so they
    // cannot show up in a build profile.
    // ------------------------------------------------------------------

    private static final java.util.concurrent.atomic.LongAdder MERGE_QUADS_IN =
            new java.util.concurrent.atomic.LongAdder();
    private static final java.util.concurrent.atomic.LongAdder MERGE_QUADS_OUT =
            new java.util.concurrent.atomic.LongAdder();
    private static final java.util.concurrent.atomic.LongAdder FAR_QUADS_IN =
            new java.util.concurrent.atomic.LongAdder();
    private static final java.util.concurrent.atomic.LongAdder FAR_QUADS_OUT =
            new java.util.concurrent.atomic.LongAdder();
    ...
    /**
     * Quads in and out, split near field against far field.
     *
     * <p>The far side is whatever came through {@link #mergeFar}, which is
     * the far mesher's three call sites and nothing else; the near side is
     * the remainder, so the two always add up to every merge that ran and
     * a new caller cannot go uncounted, only mis-attributed to the near
     * field.</p>
     *
     * <p>Read this before believing any claim about what a far-field
     * appearance row costs in geometry. A row that varies a merge channel
     * moves the far percentage and nothing else; if it does not move it,
     * it did not cost quads.</p>
     */
    public static String quadSummary() {
        long in = MERGE_QUADS_IN.sum();
        if (in == 0) {
            return "off";
        }
        long far = FAR_QUADS_IN.sum();
        long farOut = FAR_QUADS_OUT.sum();
        return String.format("near %s, far %s",
                ratio(in - far, MERGE_QUADS_OUT.sum() - farOut), ratio(far, farOut));
    }

    private static String ratio(long in, long out) {
        if (in <= 0) {
            return "none";
        }
        return String.format("%d->%d (%.1f%% merged)", in, out, 100.0 * (in - out) / in);
    }
    ...
    public static List<TerrainQuad> merge(List<TerrainQuad> quads) {
        ...
        try {
            List<TerrainQuad> merged = mergeTimed(quads);
            // Counted here rather than in the finally: a merge that threw
            // produced nothing, and folding its input into the ledger
            // would understate the rate for the rest of the session.
            MERGE_QUADS_IN.add(quads.size());
            MERGE_QUADS_OUT.add(merged.size());
            return merged;
        } finally {
    ...
    /**
     * {@link #merge}, with the result also attributed to the FAR field.
     *
     * <p>The far mesher calls this instead of {@link #merge} at all three
     * of its bucket sites. It lives here rather than as a pair of adders
     * in the far mesher so that {@link #quadSummary()} can print both
     * halves as one string, without this package - which is deliberately
     * free of every host import - having to know that a far field
     * exists.</p>
     */
    public static List<TerrainQuad> mergeFar(List<TerrainQuad> quads) {
        List<TerrainQuad> merged = merge(quads);
        FAR_QUADS_IN.add(quads.size());
        FAR_QUADS_OUT.add(merged.size());
        return merged;
    }
