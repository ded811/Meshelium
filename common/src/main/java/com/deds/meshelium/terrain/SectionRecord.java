 ...
 *               | bits 26-31 far-field scale tag (wave W3; 0 = full scale,
 *               unread by every shader until pre3 — the zero-safe argument
 *               is on the scale-tag write overload)
...
public final class SectionRecord {
    ...
    public static void write(ByteBuffer dst,
        ...
        write(dst, chunkX, chunkY, chunkZ, geomMinX, geomMinY, geomMinZ,
                geomSizeX, geomSizeY, geomSizeZ, hidden, compactedSectionId,
                0, terrainAddress, offsets);
    }
     ...
     * terrain.mesh's translucent gate, which discards everything at bit
     * 17 and above; the occlusion section raster masks {@code & 3} /
     * {@code & 0xF} — both re-verified this wave), and
     * {@code RegionStore.remove}'s swap-compaction patch masks only bits
     * 18-25, so it PRESERVES these (FARFIELD-CODEBASE-SEAM.md section
     * 3.3). Tag 0 therefore writes the exact bytes the 13-arg overload
     * always wrote; the shader-side multiply arrives in pre3.
     *
     * @param scaleTag 0-63 into header.y bits 26-31 (0 = 1:1 scale;
     *                 every pre1 caller passes 0)
     */
    public static void write(ByteBuffer dst,
                             int chunkX, int chunkY, int chunkZ,
                             int geomMinX, int geomMinY, int geomMinZ,
                             int geomSizeX, int geomSizeY, int geomSizeZ,
                             boolean hidden, int compactedSectionId,
                             int scaleTag, int terrainAddress, short[] offsets) {
        ...
        if (scaleTag < 0 || scaleTag > 63) {
            throw new IllegalArgumentException("scaleTag must be 0..63, got " + scaleTag);
        }
        ...
        // SectionManager.java:110-113 verbatim bit packing, plus the
        // wave-W3 scale tag in the free bits 26-31 (0 = bit-identical to
