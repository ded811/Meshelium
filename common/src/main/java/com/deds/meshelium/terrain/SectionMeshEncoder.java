 ...
 *   Nvidium's compat package, its mesh-result compatibility source
 *     :89-225 (stream order + offsets), :26-48 (AABB clamping), :228-247
 *     (bounds from DEQUANTIZED positions — floor/ceil of u16/2048-8)
 * Host difference: Nvidium repackaged its host mesher's already-encoded vertex
 ...
 * (that same source, 89-164) — a host-side, camera-dependent
...
public final class SectionMeshEncoder {
    ...
    private static void encodeQuadTracked(ByteBuffer dst, TerrainQuad quad, Bounds bounds) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // reads back the encoded shorts (Nvidium's compat source, 228-247).
            bounds.add(
    ...
    /** Nvidium's compat source: 23-24 sentinels, 239-247 update, 29-48 clamp. */
