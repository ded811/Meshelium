...
final class RegionStore {
     ...
     * same grid derivation above it (formula on MesheliumScaling).
     * <b>pre13:</b> and by a SECOND term the near grid knew nothing
     * about — {@code MesheliumScaling.farRegionDemand()}, the region
     * columns the far-field LOD ring spans. Until then every formula was
     * a function of rd alone, so at render distance 32 with a 120-chunk
     * LOD ring the far fill guard sat at 1,348 ids against a ring needing
     * over 3,000 and the outer band simply could not fill (see that
     * method for the worked arithmetic and the VRAM price). The scaling
     * snapshot is pinned at world standup, strictly before this
    ...
    static final class Region {
         ...
         * {@code TerrainResidency} through {@link #markRetained} and
         * {@link #setRetained}; {@link #remove} and {@link #addOrReplace}
         * clear the written position's bit unconditionally (a freshly
         * uploaded LIVE section is not retained; a removed slot cannot
         * stay marked).
         *
         * <p>Since the H2 corner-gap fix the bit no longer means only
         * "vanilla released this mesh". It means <b>vanilla's {@code
         * visibleSections} cannot be relied on to list this section</b>,
         * which is also true of a LIVE section that has left vanilla's
         * compile disc while staying inside its section-storage square
         * ({@code TerrainResidency.syncUnlistedLiveMaskLocked}). Both
         * cases want exactly the same thing from the drawer, and the mask
         * is fail-open in both: a set bit can only add a draw.</p>
         */
    ...
    /**
     * Does a section position already have an OWNER of any kind (live,
     * retained or far)?
     *
     * <p>The far-field admission path (wave W3) needs this because it
     * cannot otherwise see a LIVE resident: the live map is keyed by mesh
     * IDENTITY, so there is no position lookup, and {@link #addOrReplace}
     * is not a safe way to find out — on an occupied slot it SUCCEEDS,
     * rebinding {@code ownerByPos} and overwriting the 32-byte mirror
     * record with the caller's arena address, before the caller can
     * detect the collision. A far admission that discovered the clash
     * that late would leave the live section's slot pointing at far
     * geometry whose arena range is later recycled, i.e. permanently
     * wrong terrain drawn at a real position. This O(1) pre-flight keeps
     * that state unreachable.</p>
     */
    boolean isOccupied(long regionKey, int posKey) {
        Region region = byKey.get(regionKey);
        return region != null && region.ownerByPos[posKey] != null;
    }
    ...
    /**
     * Drive the mask bit at {@code posKey} to {@code retainedNow},
     ...
     * written on somebody else's section.</p>
     *
     * <p><b>Why a clear became necessary at all.</b> Until H2 the mask was
     * a one-way flag: it went up when a copy was RETAINED or admitted FAR,
     * and it came down only with the slot itself ({@link #remove}) or with
     * a fresh live upload at the position ({@link #addOrReplace}). Both of
     * those still hold. What H2 adds is a bit whose condition can go back
     * to FALSE while the same owner keeps the same slot: a LIVE section
     * that left vanilla's compile disc — so vanilla's {@code
     * visibleSections} stopped listing it and the BFS-mask draw path needs
     * this bit to draw it at all — and later came back inside the disc. If
     * that bit could never come down the section would keep drawing
     * unmasked for the rest of its life, losing its BFS occlusion, and the
     * marked population could only ever grow.</p>
     *
     * @return true when the stored bit changed, so the caller can bump the
     *         draw epoch only when the published mask really moved
     */
    boolean setRetained(long regionKey, int posKey, Object owner, boolean retainedNow) {
        Region region = byKey.get(regionKey);
        if (region == null || region.ownerByPos[posKey] != owner || region.pos2id[posKey] < 0) {
            return false;
        }
        int word = posKey >>> 5;
        int bit = 1 << (posKey & 31);
        boolean set = (region.retainedMask[word] & bit) != 0;
        if (set == retainedNow) {
            return false;
        }
        if (retainedNow) {
            region.retainedMask[word] |= bit;
        } else {
            region.retainedMask[word] &= ~bit;
        }
        return true;
