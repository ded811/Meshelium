...
public final class SectionBuildTap {
    ...
    /**
     * THE DISCONNECTED-TAP DETECTOR.
     *
     * <p>This class has two entry points and they are not equally
     * fragile. {@link #onCompileReturn} does all the work - it decodes
     * vanilla's mesh, encodes ours and parks it - and it hangs off a
     * mixin pinned to one exact {@code SectionCompiler.compile}
     * descriptor. {@link #onMeshConstructed} only collects what was
     * parked, and it hangs off a constructor.
     *
     * <p>If the compile tap ever stops matching, nothing throws. The
     * mixin still applies, {@code defaultRequire = 1} is still satisfied,
     * no line is logged, nothing is ever parked, the arena stays empty
     * and {@code TerrainDrawer} quietly hands every frame back to
     * vanilla. The player sees a normal world at vanilla frame rate and
     * we see a green suite. That is the 1.2.0 invisible-terrain shape and
     * it is the worst failure this renderer has.
     *
     * <p>It is not hypothetical. NeoForge 26.2 keeps the 4-argument
     * {@code compile} as a deprecated delegating shim and repoints its
     * only caller at a new 5-argument overload, so on that loader this
     * tap is disconnected by construction.
     *
     * <p>The detector costs two counters and one branch. The constructor
     * tap firing proves vanilla is building sections; the compile tap
     * never having fired proves our decode path is not attached to them.
     * Those two facts together can only mean the anchor is wrong, and
     * saying so once is the difference between a named error and a
     * mystery.
     */
    private static final java.util.concurrent.atomic.AtomicLong COMPILE_TAP_HITS =
            new java.util.concurrent.atomic.AtomicLong();

    /** See {@link #COMPILE_TAP_HITS}. Mesh-ctor hits with nothing parked. */
    private static final java.util.concurrent.atomic.AtomicLong MESH_TAP_ORPHANS =
            new java.util.concurrent.atomic.AtomicLong();

    /** See {@link #COMPILE_TAP_HITS}. Latches the one-time report. */
    private static final java.util.concurrent.atomic.AtomicBoolean TAP_BREAK_REPORTED =
            new java.util.concurrent.atomic.AtomicBoolean();

    /**
     * How many mesh constructions may pass with nothing parked and no
     * compile tap ever seen before we call it broken. Sized well above
     * any startup race: the two taps run on the same thread within one
     * build task, so a healthy build can never produce a long run of
     * these, while a disconnected tap produces nothing else.
     */
    private static final long TAP_BREAK_THRESHOLD = 256L;

    /** See {@link #COMPILE_TAP_HITS}: the compile tap's lifetime count. */
    public static long compileTapHits() {
        return COMPILE_TAP_HITS.get();
    }

    /** See {@link #COMPILE_TAP_HITS}: mesh constructions with nothing parked. */
    public static long meshTapOrphans() {
        return MESH_TAP_ORPHANS.get();
    }
    ...
    /**
     * P2: {@code levelRenderer.visibleSections().size()}, sampled by the
     * drawer at frame HEAD and parked here for the same reason the camera
     * section is — {@code TerrainResidency}'s stats line wants it and may
     * not name the LWJGL-importing {@code TerrainDrawer} to get it.
     *
     * <p>It is the fly-up defect measured at its source: nothing is
     * unloaded on a vertical move, vanilla just stops LISTING, and an
     * unlisted section is never compiled either
     * ({@code LevelExtractor.extract} scans this list). −1 until a frame
     * has published one.</p>
     */
    private static volatile int visibleSectionsListed = -1;
    private static volatile int visibleSectionsListedMin = -1;
    private static volatile int visibleSectionsListedMax = -1;

    /** Render thread, once per frame: the size of vanilla's BFS output list. */
    public static void publishVisibleSections(int listed) {
        visibleSectionsListed = listed;
        int min = visibleSectionsListedMin;
        if (min < 0 || listed < min) {
            visibleSectionsListedMin = listed;
        }
        if (listed > visibleSectionsListedMax) {
            visibleSectionsListedMax = listed;
        }
    }

    /** Any thread: vanilla's BFS output size last frame, or −1. */
    public static int visibleSectionsListed() {
        return visibleSectionsListed;
    }

    /** Any thread: session low-water of {@link #visibleSectionsListed()}. */
    public static int visibleSectionsListedMin() {
        return visibleSectionsListedMin;
    }

    /** Any thread: the session high-water, so the collapse reads as a ratio. */
    public static int visibleSectionsListedMax() {
        return visibleSectionsListedMax;
    }
    ...
    public static void onCompileReturn(SectionPos pos, SectionCompiler.Results results) {
        ...
        // TWO ARMS, ONE BUILD. The mixin taps both the four- and the
        // five-argument compile, because Fabric has only the former and
        // NeoForge routes its callers through the latter. On NeoForge a
        // legacy caller can reach the deprecated four-argument shim, which
        // delegates - so the inner arm returns and parks, then the outer
        // arm returns with the SAME Results object. Drop the second one by
        // identity rather than decoding the section twice.
        Parked already = PARKED.get();
        if (already != null && already.results() == results) {
            return;
        }
        com.deds.meshelium.MesheliumCpuStages.noteTapCompile();
        COMPILE_TAP_HITS.incrementAndGet(); // see COMPILE_TAP_HITS
    ...
    public static void onMeshConstructed(Object mesh, SectionCompiler.Results results) {
        ...
        if (parked == null) {
            // Empty section, 0-quad decode, or tap disabled mid-build -
            // all normal. A LONG RUN of these with the compile tap never
            // having fired once is not: see COMPILE_TAP_HITS.
            if (COMPILE_TAP_HITS.get() == 0L
                    && MESH_TAP_ORPHANS.incrementAndGet() >= TAP_BREAK_THRESHOLD
                    && TAP_BREAK_REPORTED.compareAndSet(false, true)) {
                com.deds.meshelium.MesheliumLog.LOGGER.error(
                        "Meshelium's section-compile tap is DISCONNECTED: vanilla has built {} "
                                + "sections and SectionCompiler.compile has not been seen once, "
                                + "so nothing is being decoded, nothing is uploaded, and every "
                                + "frame is being handed back to vanilla. The world will look "
                                + "correct and run at vanilla speed. This means the mixin's "
                                + "pinned compile(...) descriptor no longer matches the game - "
                                + "check SectionCompilerMixin against this Minecraft build "
                                + "(a loader that adds a parameter to compile, or repoints its "
                                + "caller at an overload, produces exactly this).",
                        MESH_TAP_ORPHANS.get());
            }
            return;
        }
        PARKED.remove();
