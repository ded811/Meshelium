 ...
 *   encoder: Nvidium's compat package, NvidiumCompactChunkVertex.java (lines 21-96)
 ...
 *       lightmap-edge guard Nvidium inherited from its host mesher's convention
 *       (NvidiumCompactChunkVertex.java:66-71). GLSL reads the pair as
 *       {@code uvec2(v.y>>24, v.z>>24) / 256.0} (vertex_format.glsl:41-44).</li>
 * </ul>
 *
 * <h2>Named upstream-constant assumptions</h2>
 * <ul>
 *   <li>The [-8, 24) position domain is the upstream chunk-mesh vertex space
 ...
 *       shaders as TEXTURE_MAX_SCALE; it assumed the upstream ChunkVertexType
 *       textureScale contract (getTextureScale() = 1/32768).</li>
 *   <li>Colour premultiply: Nvidium multiplies R/G/B by the alpha channel
 *       (the upstream AO brightness) via ColorU8 float round-trips and zeroes
 *       alpha. That ColorU8 source is NOT in the reference tree, so the
 *       exact rounding is UNVERIFIED; this codec uses
 *       {@code Math.round(channel * alpha/255f)} (round-half-up), which can
 *       differ from the upstream encoder's by at most 1 LSB per channel. Pinned by test.</li>
 *   <li>The material-bit VALUES (which cutoff index a given pass uses) came
 *       from repackaging the upstream Material.bits(); Meshelium's wave-3b encoder
 *       chooses them itself: solid/translucent = mip + cutoff 0
 *       (Nvidium's compat source, 112 and 185), cutout = mip flag +
...
public final class TerrainVertexCodec {
     ...
     * Nvidium's compat source, 112, 185 and 202-211).
     */
     ...
     * ABGR {@code 0xAABBGGRR} (vanilla channel order: R in the low
     ...
     * (Nvidium's compat source, 110-113 and 183-189).
     */
