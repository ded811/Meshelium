...
// The pre16 far-cutout mip clamp (FAR_CUTOUT_MAX_LOD, keyed on mip bit
// clear + cutoff CUTOUT) lived here and is DELETED at pre18 (R1): the
// coverage table that justified it omitted vanilla's per-sprite
// alpha_cutoff_bias (kelp and kelp_plant declare 0.1) and the +0.025
// floor scaleAlphaToCoverage adds to every mip texel, and with the full
// chain modelled the clamp pinned far kelp at its WORST level (kelp_plant
// 52% half-on dither at mip 2) while forbidding vanilla's own far
// behaviour (solid at mip 3, gone at mip 4 - 0% of texels pass 0.5
// there). Far cutout quads now sample exactly what the near field
